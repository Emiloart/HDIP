...
func Connect(driverName string, dsn string) (*Store, error) {
	...
	if normalizedDSN == "" {
		return nil, fmt.Errorf("phase1 sql dsn must not be empty")
	}

	db, err := sql.Open(normalizedDriver, normalizedDSN)
	if err != nil {
